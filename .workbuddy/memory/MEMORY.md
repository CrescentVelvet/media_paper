...
- **`Supplementary` 链接要实际 GET 验证存活**(本次实测 404)。项目页的 publication-links 区域可用正则一次提全部 `<a href>`。
- **论文图号 ≠ 图片文件出现序**。准确映射必须**按 `<figure>` 块逐块提** `id` + `<img src>` + `<figcaption>`。有些 Figure(LaTeXML 下的 TikZ 矢量图)块内 `<img>` 为空 → 下载清单天然比图号少一张,**不是漏下**;用 Mermaid 在笔记里重述其逻辑补位。
- **相对改进百分比要逐个按表格原始值手算**:论文把表格数字二次加工成结论句时最易出错,且常错在**对比对象**而非数字。本次 LiON-LoRA §4.3 称 FVD "较 CamI2V 降 37.8%",按表 1 CamI2V=294.6 实为 53.8%,37.8% 恰与 CameraCtrl(218.9)吻合。笔记里用"论文表述 / 表格复核值 / 结论"三列并排列出,是本库很有价值的原创发现形式。

## 3. 文档同步清单(改规范时必查)
改 `AGENTS.md` 的规范条目后,按下表逐项排查,否则会留下自相矛盾的文档:
- `README.md`:规范描述、笔记篇数(现 68)、维护流程小节。
- `Meta-20260813-用AI做论文调研经验分享.html`(原 `CV-AI-20260813-...`,2026-09-22 前缀体系重构改名):3.2 模块清单 / 3.3 验证清单 / 3.4 样式选型 / §9 索引设计 —— 已按统一格式改写(commit c05de7e),后续改规范仍需复查。
